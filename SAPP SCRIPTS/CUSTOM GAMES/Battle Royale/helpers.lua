...
local read_float, read_dword, read_vector3d, read_byte = read_float, read_dword, read_vector3d, read_byte

-- Returns the player's position (x, y, z) and whether they are in a vehicle
...
-- Checks if the distance squared between two points is within a given radius
function HELPERS.distance_squared(x1, y1, z1, x2, y2, z2, radius)
    local dx, dy, dz = x1 - x2, y1 - y2, z1 - z2
    return (dx * dx + dy * dy + dz * dz) <= (radius * radius)
end

-- Returns the player's inventory {weapons, grenades, etc.}
...
-- Validates if a player is present and alive
...
function HELPERS:send(player_id, ...)
    if not player_id then
        ...
        say_all(format(...))
        execute_command('msg_prefix "' .. self.MSG_PREFIX .. '"') -- restore prefix
        return
    end
...
-- Retrieves the tag ID of a given class and name
...
-- Debug print if DEBUG is enabled
